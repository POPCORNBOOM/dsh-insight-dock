...
agent 在专注做一件事的时候，可以把一个简短、边界严格的侧面发现先寄存在这里——「欸，另外那件事其实可以…」——而不打断手上的活。纸条会出现在输入框上方，你之后再决定要不要听它讲。

关键在于**上下文锚点**。每张纸条都记住自己被创建时所在的会话序号，所以 `insight_load` 能从那一刻把真实的对话日志读回来。那次回读走的是持久化的会话日志，**因此哪怕 agent 自己的上下文已经被压缩掉，纸条依然有效**。
...
**输入框上方**是一行坞——`💡 3 条见解`——展开就是当前这张纸条，箭头用来翻其余几条。

| 动作 | 效果 |
| --- | --- |
| 说来听听 | 让 agent 顺着它锚定的那段日志，讲讲自己当时的想法。 |
| 关闭 | 移除这张纸条。 |
| 关闭并告诉 agent 为什么 | 移除纸条，**并且**把你的原因作为一行 notice 注入给 agent。 |
...
| 等级 | 注入时机 | 内容 |
| --- | --- | --- |
| 不邀请 | 从不——但你「关闭并说明原因」的通知仍会送达 | — |
| 有纸条时 | 只在已经有纸条时 | 那条纸条提醒 |
| 一直邀请 | 一直 | 正向判据：*真实、现在做不了、不记就永远丢掉* |
| 一直邀请 + 强化 | 一直 | 上述判据，外加一句「每次开工前先扫一眼」 |

默认是「有纸条时」，即保持原行为——而它**在结构上不可能产生第一条纸条**：它据以注入的那条提醒，前提是已经有一条被记下来了。上面两级在零纸条时也注入，那是第一条能出现的唯一途径。

这个文本的每一个早期版本都是一串禁令——*不是待办*、*不是暂缓工作的出口*、*现在能做就现在做*。实测结果就是弃权：**94 个会话、625 轮里，被记录过的纸条全部是开发这个插件时造出来的**——17 次调用，每一次都是测试夹具，其余 93 个会话是 **0**。一份完全由「不要」构成的说明，产出的正是这个。所以「一直邀请」这两级改成陈述正向判据。
...
| 表面 | 作用 |
| --- | --- |
| `insight_add` | 记录一张纸条。标题 ≤ 60、理由 ≤ 200、必填的 `why_not_now` ≤ 120、一轮最多一条、同时最多 5 条。 |
| `insight_list` | 列出当前会话的纸条。 |
| `insight_load` | 读回纸条锚点附近的那段日志——**并消费掉这张纸条**。 |
| `insight_withdraw` | 让 agent 撤回自己留下的纸条。 |
| prompt context | 有纸条时注入一行环境提示：这些不是待办；如果其中某条其实属于当前任务，就必须去**做掉**，而不是归档。 |
...
| `heard` → 消失 | agent 用 `insight_load` 读走它 | 上下文已经交付，纸条随之移除 |
...
所以一张被听过的纸条通常只活**几秒**而不是几分钟：`insight_load` 在把内容交出去的那一瞬间就删掉它。`heardAt` 那条存活时间只是兜底，防止没人应答的纸条永远赖在坞里。

**纸条只存在内存里。** 关于纸条的任何东西都不会写到磁盘。在"存活时间以分钟计"的前提下这几乎没有代价——重启本来就比任何纸条活得久，持久化只可能保住一个空集合——而且它消掉了一整类失效：静默失败的写入、`serial` 记账，以及 side-car 会带来的 fork / workspace / 存储层那一串坑。
...
2. **每一种结束都删除，没有归档。** 不存在「已归档」那一行，也没有它背后的记录。过期尤其不可逆：调短存活时间会直接销毁纸条，**再调长也救不回来**。

3. **重启会清空这个坞。** 纸条存在内存里，所以活不过 `dsh` 重启，fork 出来的会话也不继承。默认 5 分钟存活时间下这一点看不见；只有当你把存活时间调得很长、**并且**在纸条还开着的时候重启，才会撞上。

   把纸条改成日志驻留——用 `session.append` 写一个只进日志、不面向界面的状态事件，再 `sessionProjections.register`——就能做到 fork 继承与 checkpoint。但那需要用 TypeScript 模块增强去注册事件类型，这是手写 `lib/*.js` 的包做不到的。**这是刻意不付的债**：存活时间以时间为基础时，持久化买不到多少东西。
...
当你**附带原因**关闭一张纸条时，host 会向 agent 注入一条单行 `notice`——**刻意不唤醒 driver**，所以 agent 无法回复它。不带原因关闭则谁也不通知。
