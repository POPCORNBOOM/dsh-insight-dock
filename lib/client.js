			...
			function clearCache() {
				api("clear", {})
					.then((r) => {
						...
						setClearMsg("已清除 " + String(r.cleared) + " 条。见解只在内存里，没有文件可清。")
					})
					.catch(() => {
						setConfirming(false)
						setClearMsg("清除失败。")
			...
			return React.createElement(
				...
				React.createElement(
					...
					React.createElement(
						"div",
						{ className: "ins-rowText" },
						React.createElement("div", { className: "ins-rowTitle" }, "清除见解缓存"),
						React.createElement(
							"div",
							{ className: "ins-rowDesc" },
							clearMsg || "清掉当前进程里的全部见解记录。见解只存在于内存，重启即空。不可撤销。",
						),
					),
					React.createElement(
						"button",
						{
							className: "ins-btn" + (confirming ? " ins-btn-primary" : ""),
							type: "button",
							onClick: () => {
								if (confirming) clearCache()
