window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			".ins-select{box-sizing:border-box;height:36px;min-width:136px;padding:0 12px;border:none;border-radius:18px;background:var(--dsw-alias-bg-module-platform);color:var(--dsw-alias-label-primary);font:inherit;font-size:14px;line-height:22px;cursor:pointer}",
			".ins-select:focus-visible{outline:1px solid var(--dsw-alias-state-business-primary);outline-offset:1px}",
		...
		const INJECTION_OPTIONS = [
			{ value: "off", label: "不邀请", desc: "不注入任何邀请。已有纸条时，你关闭纸条附的原因仍然送达。" },
			{
				value: "whenOpen",
				label: "有纸条时",
				desc: "只在已经有纸条时注入一句提醒。注意：它在结构上无法促成第一条——提醒的前提是先有一条。",
			},
			{
				value: "always",
				label: "一直邀请",
				desc: "无论有没有纸条都常驻一句正向判据（真实、现在做不了、不记就会丢）。这是第一条能出现的前提。",
			},
			{
				value: "insist",
				label: "一直邀请 + 强化",
				desc: "在「一直邀请」之上再加一句：每次开工前先扫一眼有没有这样的观察。",
			},
		]
				...
				React.createElement(
					...
					React.createElement(
						"select",
						{
							...
							value: cfg.injection,
							onChange: (e) => patch({ injection: e.target.value }),
						},
