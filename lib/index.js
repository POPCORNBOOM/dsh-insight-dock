...
/** Ambient-prompt injection levels, weakest first.
 *
 *  `whenOpen` is the historical behaviour and is structurally unable to cause a
 *  FIRST note: the open-note reminder it gates on does not exist until one has
 *  already been recorded. `always` and `insist` inject an invitation even with
 *  zero notes, which is the only way the first one can ever appear. */
const INJECTION_LEVELS = ['off', 'whenOpen', 'always', 'insist']
const DEFAULT_INJECTION = 'whenOpen'
...
export function apply(ctx) {
  ...
  function applyConfig(raw) {
    ...
    if (raw.injection !== undefined && INJECTION_LEVELS.indexOf(String(raw.injection)) !== -1) {
      config.injection = String(raw.injection)
    }
  }
  ...
  const toolList = defineTool({
    name: 'insight_list',
    description: '查看当前会话里你留下的见解（标题、理由、创建多久了）。用于避免重复记录，以及确认自己是否已经提过、有没有被用户关掉。',
    parameters: {},
    output: TOOL_TEXT_OUTPUT,
    execute: async (_args, exec) => {
      ...
      if (!all.length) lines.push('当前会话没有见解。')
      else {
  ...
  /** The POSITIVE criterion. Every earlier version of this text was a list of
   *  prohibitions, which produces abstention; this one says when recording is
   *  the right call, in the agent's own terms. */
  const INVITE =
    '你在专注时可以把与手上任务无关的侧面发现记成一条见解（insight_add）：它真实、你现在做不了、不记就会永远丢掉——三条都满足就记下来，不必等到被打断。'

  /** `insist` adds a standing check rather than only a criterion. */
  const INVITE_INSIST = INVITE + '每次开工前先扫一眼有没有这样的观察；一条都没有就不用管这句。'
      ...
      promptSvc.context({
        ...
        text: (context) => {
          try {
            ...
            const level = config.injection
            const sid = assemblySessionId(context)
            if (!sid) return ''
            const parts = []

            // A close-with-reason notice is a delivery, not an invitation: `off`
            // suppresses the invitation only, so a user-initiated notice still
            // reaches the agent even at the weakest level.
            const notice = takeNotices(sid)
            if (notice) parts.push(notice)

            if (level === 'always' || level === 'insist') {
              parts.push(level === 'insist' ? INVITE_INSIST : INVITE)
            }
